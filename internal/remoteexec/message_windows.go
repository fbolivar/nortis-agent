...
// EjecutarMensaje muestra un aviso en la pantalla del usuario. Reutiliza el
...
func EjecutarMensaje(ctx context.Context, p MessagePayload) (int, string, error) {
	...
	if err != nil {
		return -1, "", fmt.Errorf("no hay un usuario con sesion activa al que avisar: %w", err)
	}
	defer token.Close()
	...
	cmd := exec.CommandContext(ctx, exe, "notify", "-title", titulo, "-message", p.Body)
	...
	// no hay que bloquear la tarea esperando a que lo lea.
	if err := cmd.Start(); err != nil {
		return -1, "", fmt.Errorf("no se pudo mostrar el aviso: %w", err)
	}
	go func() { _ = cmd.Wait() }()
